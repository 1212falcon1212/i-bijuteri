...
function SettlementCard({
  ...
  return (
    <div className={cn(
      ...
      isOpen ? 'border-[#D4B896]' : 'border-[#f0eceb] hover:border-[#D4B896]'
    )}>
      {/* Header */}
      <button
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {isUpcoming ? (
              ...
              <span className="inline-flex items-center text-[10px] font-semibold px-3 py-1 rounded-full bg-[#F1E6D0] text-[#B89968]">
                Ödeme Yapildi
              </span>
            )}
          </div>
          ...
          <div className="p-4">
            {loadingDetails ? (
                ...
                <div className="flex gap-1 bg-[#faf8f6] rounded-xl p-1 mb-5 w-fit">
                  <button
                    ...
                    className={cn(
                      ...
                      viewMode === 'summary'
                        ? 'bg-white text-[#B89968] border border-[#f0eceb]'
              ...
              <div className="text-center py-6">
                <FileText className="w-10 h-10 mx-auto text-charcoal-light mb-2" />
                <p className="text-sm text-charcoal-light">Detay bilgisi yüklenemedi</p>
                {loadError && (
                ...
                <button
                  ...
                  className="text-xs text-[#B89968] hover:text-[#8C6F3F] font-medium mt-2"
                >
                  Tekrar Dene
                </button>
              </div>
            ...
            return (
              ...
              >
                ...
                <td className={cn(
                  ...
                  isTotal && 'font-medium text-[#B89968]'
                )}>
                ...
                <td className={cn(
                  ...
                  isTotal
                    ? 'text-[#B89968] text-base'
                    : row.type === 'credit' ? 'text-[#B89968]' : 'text-red-500'
                )}>
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-8">
        <Package className="w-12 h-12 mx-auto text-charcoal-light mb-3" />
        <p className="text-sm text-charcoal-light">Sipariş bulunamadi</p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex justify-end">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold text-[#B89968] bg-[#F1E6D0] hover:bg-[#F1E6D0] border border-[#f0eceb] rounded-xl transition-colors"
        >
      ...
      <div className="hidden lg:block border border-[#f0eceb] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#faf8f6] border-b border-[#f0eceb]">
                <th className="text-left py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Sipariş</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Sipariş Tutari</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Komisyon</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Stopaj</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Kargo</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Toplam Kesinti</th>
                <th className="text-right py-3 px-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Net Tutar</th>
                <th className="text-center py-3 px-2 text-xs font-semibold text-charcoal-light uppercase tracking-wider w-12"></th>
              </tr>
            </thead>
            <tbody>
              {items.map((order, i) => (
                <Fragment key={order.order_number}>
                  <tr
                    className={cn(
                      'border-b border-card-border last:border-0 hover:bg-surface-bg/50 transition-colors',
                      expandedOrder === order.order_number && 'bg-surface-bg/50'
                    )}
                  >
                    <td className="py-3.5 px-4">
                      <p className="font-semibold text-charcoal">{order.order_number}</p>
                      <p className="text-xs text-charcoal-light mt-0.5">{order.order_date}</p>
                    </td>
                    <td className="py-3.5 px-4 text-right font-medium text-charcoal-mid whitespace-nowrap">
                    ...
                    <td className="py-3.5 px-4 text-right font-bold text-[#B89968] whitespace-nowrap">
                    ...
                    <td className="py-3.5 px-2 text-center">
                      {order.items && order.items.length > 0 && (
                        ...
                        >
                          <ChevronDown className={cn(
                            'w-4 h-4 text-charcoal-light transition-transform duration-200',
                  ...
                  {expandedOrder === order.order_number && order.items && order.items.length > 0 && (
                    <tr>
                      <td colSpan={8} className="bg-surface-bg/80 px-4 py-3 border-b border-card-border">
                        <p className="text-[10px] font-semibold text-charcoal-light uppercase tracking-wider mb-2">Ürünler</p>
                        <div className="space-y-1.5">
                          {order.items.map((product, pi) => (
                            <div key={pi} className="flex items-center justify-between text-xs">
                              <span className="text-charcoal-mid">{product.product_name}</span>
                              <span className="text-charcoal-light whitespace-nowrap ml-4">
                                {product.quantity} adet x {formatMoney(product.unit_price)} = <span className="font-medium text-charcoal-mid">{formatMoney(product.total_price)}</span>
                              </span>
                            </div>
                          ))}
                        </div>
            ...
            {items.length > 1 && (
              <tfoot>
                <tr className="bg-[#F1E6D0] border-t border-[#f0eceb]">
                  <td className="py-3.5 px-4 font-semibold text-charcoal-mid">
                    Toplam ({items.length} sipariş)
                  </td>
                  <td className="py-3.5 px-4 text-right font-bold text-charcoal-mid whitespace-nowrap">
      ...
      <div className="hidden md:block lg:hidden border border-[#f0eceb] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#faf8f6] border-b border-[#f0eceb]">
                <th className="text-left py-3 px-3 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Sipariş</th>
                <th className="text-right py-3 px-3 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Tutar</th>
                <th className="text-right py-3 px-3 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Kesinti</th>
                <th className="text-right py-3 px-3 text-xs font-semibold text-charcoal-light uppercase tracking-wider">Net</th>
                <th className="w-10" />
              </tr>
            </thead>
            <tbody>
              {items.map((order) => (
                <Fragment key={order.order_number}>
                  <tr
                    className="border-b border-card-border last:border-0 hover:bg-surface-bg/50 transition-colors"
                  >
                    <td className="py-3 px-3">
                      <p className="font-semibold text-charcoal text-sm">{order.order_number}</p>
                      <p className="text-xs text-charcoal-light">{order.order_date}</p>
                    </td>
                    <td className="py-3 px-3 text-right font-medium text-charcoal-mid whitespace-nowrap text-sm">
                      {formatMoney(order.total_price)}
                    </td>
                    <td className="py-3 px-3 text-right text-red-500 whitespace-nowrap text-sm">
                      -{formatMoney(totalDeduction(order))}
                    </td>
                    <td className="py-3 px-3 text-right font-bold text-[#B89968] whitespace-nowrap text-sm">
                      {formatMoney(order.net_amount)}
                    </td>
                      ...
                      <td colSpan={5} className="bg-surface-bg/80 px-3 py-3 border-b border-card-border">
                        ...
                        <div className="space-y-1.5">
                          {order.items.map((product, pi) => (
                            <div key={pi} className="flex items-center justify-between text-xs">
                              <span className="text-charcoal-mid">{product.product_name}</span>
                              <span className="text-charcoal-light whitespace-nowrap ml-2">
            ...
            {items.length > 1 && (
              <tfoot>
                <tr className="bg-[#F1E6D0] border-t border-[#f0eceb]">
                  <td className="py-3 px-3 font-semibold text-charcoal-mid text-sm">Toplam</td>
                  <td className="py-3 px-3 text-right font-bold text-charcoal-mid whitespace-nowrap text-sm">
                    {formatMoney(items.reduce((s, o) => s + o.total_price, 0))}
                  </td>
      ...
      <div className="md:hidden space-y-3">
        {items.map((order) => (
          <div key={order.order_number} className="border border-[#f0eceb] rounded-2xl overflow-hidden bg-white">
            <button
              onClick={() => setExpandedOrder(expandedOrder === order.order_number ? null : order.order_number)}
              className="w-full p-4 text-left hover:bg-surface-bg/50 transition-colors"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="font-semibold text-charcoal">{order.order_number}</p>
                  <p className="text-xs text-charcoal-light mt-0.5">{order.order_date} &middot; {order.item_count} kalem</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="text-right">
                    <p className="text-[10px] text-charcoal-light">Net</p>
                    <p className="font-bold text-[#B89968]">{formatMoney(order.net_amount)}</p>
                  </div>
                  <ChevronDown className={cn(
                    'w-4 h-4 text-charcoal-light transition-transform duration-200',
                    expandedOrder === order.order_number && 'rotate-180'
              ...
              <div className="flex items-center gap-2 mt-2 text-xs flex-wrap">
                <span className="text-charcoal-mid font-medium">{formatMoney(order.total_price)}</span>
              ...
              <div className="border-t border-card-border px-4 py-3 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2.5 bg-surface-bg rounded-lg text-center">
                    <p className="text-[10px] text-charcoal-light uppercase font-semibold">Komisyon</p>
                    <p className="text-sm font-bold text-red-500 mt-0.5">
                      {order.service_fee > 0 ? `-${formatMoney(order.service_fee)}` : '-'}
                    </p>
                  </div>
                  <div className="p-2.5 bg-surface-bg rounded-lg text-center">
                    <p className="text-[10px] text-charcoal-light uppercase font-semibold">Stopaj</p>
                    <p className="text-sm font-bold text-red-500 mt-0.5">
                      {order.withholding_tax > 0 ? `-${formatMoney(order.withholding_tax)}` : '-'}
                    </p>
                  </div>
                  <div className="p-2.5 bg-surface-bg rounded-lg text-center">
                    <p className="text-[10px] text-charcoal-light uppercase font-semibold">Kargo</p>
                    <p className="text-sm font-bold text-red-500 mt-0.5">
                      {order.shipping_share > 0 ? `-${formatMoney(order.shipping_share)}` : '-'}
                    </p>
                  </div>
                ...
                {order.items && order.items.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {order.items.map((product, pi) => (
                        <div key={pi} className="flex items-center justify-between text-xs">
                          <span className="text-charcoal-mid truncate mr-2">{product.product_name}</span>
        ...
        {/* Mobile Total */}
        {items.length > 1 && (
          <div className="flex items-center justify-between px-4 py-3 bg-[#F1E6D0] rounded-xl border border-[#f0eceb]">
            <span className="text-sm font-semibold text-charcoal-mid">Toplam ({items.length} sipariş)</span>
            <span className="font-bold text-[#B89968]">{formatMoney(items.reduce((s, o) => s + o.net_amount, 0))}</span>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className="text-center py-16 bg-[#faf8f6] rounded-2xl border border-[#f0eceb]">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-[#B89968] hover:bg-[#8C6F3F] rounded-xl transition-colors"
        >
          Tekrar Dene
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="border border-[#f0eceb] rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-7 h-7 rounded-lg bg-[#F1E6D0] flex items-center justify-center">
              <TrendingUp className="w-3.5 h-3.5 text-[#B89968]" />
            </div>
        ...
        <div className="p-4 bg-[#1a1a1a] rounded-2xl text-white">
          ...
          <div className="sm:hidden space-y-2">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/10">
              <span className="text-xs text-[#D4B896] font-semibold">Tahmini Net</span>
              <span className="text-base font-black text-[#D4B896]">{formatMoney(upcomingSummary.net_estimated_total)}</span>
            </div>
          </div>
          {/* Desktop: grid */}
          <div className={`hidden sm:grid gap-4 text-center ${(upcomingSummary as any).total_refunds > 0 ? 'grid-cols-6' : 'grid-cols-5'}`}>
            ...
            <div className="space-y-1 border-l border-white/10 pl-4">
              <p className="text-[10px] text-[#D4B896] uppercase tracking-wider font-semibold">Tahmini Net</p>
              <p className="text-base font-black text-[#D4B896]">{formatMoney(upcomingSummary.net_estimated_total)}</p>
            </div>
          </div>
        </div>
      )}
