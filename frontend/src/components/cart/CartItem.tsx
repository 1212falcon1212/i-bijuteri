...
function ProductImage({ src, alt, className }: { src: string | null | undefined; alt: string; className?: string }) {
    ...
    if (!src || error) {
        return (
            <div className={cn("bg-surface-bg dark:bg-slate-700 flex items-center justify-center", className)}>
                <Box className="h-10 w-10 text-charcoal-light" />
            </div>
        );
    }
...
export function CartItem({
    ...
    return (
        <div
            className={cn(
                "group relative flex gap-4 p-4 rounded-xl bg-white dark:bg-charcoal border transition-all duration-300",
                issue ? "border-red-200 bg-red-50/50 dark:bg-red-900/10" : "border-card-border dark:border-slate-700",
        ...
        >
            ...
            <div className="flex-1 min-w-0 space-y-2">
                {/* Title & Brand */}
                <div>
                    <h3 className="font-semibold text-charcoal dark:text-white line-clamp-2 group-hover:text-primary transition-colors">
                        {item.product.name}
                    </h3>
                    {item.product.brand && (
                        <p className="text-sm text-charcoal-light">{item.product.brand}</p>
                    )}
                </div>
                ...
                {item.offer.expiry_date && (
                    <div className={cn(
                        ...
                        isExpiringSoon ? "text-amber-600" : "text-charcoal-light"
                    )}>
                ...
                <div className="flex items-center justify-between pt-2">
                    ...
                    <div className="text-sm">
                        <span className="text-charcoal-light">Birim: </span>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isEditing ? (
                            ...
                            <button
                                className="w-10 h-8 text-center font-semibold text-charcoal dark:text-white hover:bg-surface-bg dark:hover:bg-slate-700 rounded-md transition-colors"
            ...
            <div className="flex flex-col items-end justify-between">
                ...
                <Button
                    size="icon"
                    variant="ghost"
                    className="h-8 w-8 text-charcoal-light hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-opacity"
