...
function ProductImage({ src, alt }: { src: string | null | undefined; alt: string }) {
    ...
    if (!src || error) {
        return (
            <div className="w-12 h-12 bg-card-border dark:bg-slate-700 rounded-md flex items-center justify-center flex-shrink-0">
                <Box className="h-5 w-5 text-charcoal-light" />
            </div>
        );
    }
        ...
        <>
            ...
            <button
                ...
                onClick={() => {
                    if (itemCount > 0 && !isCartPage) {
                        ...
                        // empty cart — open mini panel for "ürün ekleyin" hint
                        setIsOpen(true);
                    }
                }}
            >
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
                <SheetContent side="right" className="w-full sm:max-w-[420px] flex flex-col p-0 gap-0">
                    {/* Header */}
                    <SheetHeader className="p-4 bg-surface-bg dark:bg-charcoal/50 border-b border-card-border dark:border-slate-700 space-y-0">
                        <div className="flex items-center gap-2">
                            <ShoppingCart className="h-5 w-5 text-[#B89968]" />
                    ...
                    {itemCount === 0 ? (
                        <div className="flex flex-col items-center justify-center flex-1 py-12 px-4 text-center">
                            <div className="w-20 h-20 rounded-full bg-surface-bg dark:bg-charcoal flex items-center justify-center mb-4">
                                <ShoppingCart className="h-10 w-10 text-charcoal-light" />
                            </div>
                        ...
                        <>
                                        ...
                                        <div key={group.seller?.id || 'unknown'} className="space-y-2">
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-charcoal-light">
                                                ...
                                                {group.seller?.id ? (
                                                    <Link
                                                        ...
                                                        className="font-medium text-charcoal-mid dark:text-charcoal-light hover:text-[#B89968] hover:underline transition-colors"
                                                        onClick={() => setIsOpen(false)}
                                                    >
                                                        {group.seller.nickname || group.seller.business_name || 'Satıcı'}
                                                    </Link>
                                                ) : (
                                                    <span className="font-medium text-charcoal-mid dark:text-charcoal-light">
                                                        {group.seller?.nickname || group.seller?.business_name || 'Satıcı'}
                                                    </span>
                                                )}
                                                {group.seller?.city && (
                                                    <span className="text-charcoal-light">({group.seller.city})</span>
                                                )}
                                            </div>
                                            ...
                                            {group.items.map((item) => (
                                                <div
                                                    key={item.id}
                                                    className={cn(
                                                        "flex gap-3 p-2 rounded-lg bg-surface-bg dark:bg-charcoal/50 transition-all duration-300",
                                                        lastAddedItemId === item.id && "ring-2 ring-[#B89968] ring-offset-2 bg-[#FBF8F3]",
                                                ...
                                                >
                                                            ...
                                                            if (priceIssue && priceIssue.old_price && priceIssue.new_price) {
                                                                return (
                                                                    <div className="flex items-center gap-1.5">
                                                                        <span className="text-xs text-charcoal-light line-through">{formatPrice(priceIssue.old_price)}</span>
                                                                        <span className={cn("text-xs font-medium", priceIssue.new_price > priceIssue.old_price ? "text-red-600" : "text-[#B89968]")}>
                                                                            {formatPrice(priceIssue.new_price)}
                                                                        </span>
                                                                    </div>
                                                                );
                                                            }
                                                            return <p className="text-xs text-charcoal-light">{formatPrice(item.price_at_addition)}</p>;
                                                        })()}
                                                    ...
                                                    <Button
                                                        size="icon"
                                                        variant="ghost"
                                                        className="h-6 w-6 text-charcoal-light hover:text-red-500 self-start"
                            ...
                            {/* Footer */}
                            <div className="flex-shrink-0 border-t border-card-border dark:border-slate-700 p-4 space-y-3 bg-surface-bg dark:bg-charcoal/50">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-charcoal-mid dark:text-charcoal-light">Ara Toplam:</span>
                                ...
                                <div className="flex flex-col gap-2">
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            className="flex-1 bg-[#B89968] hover:bg-[#8C6F3F] text-white"
                                            asChild
                                            onClick={() => setIsOpen(false)}
                                        >
                                            <Link href="/checkout">
                                    ...
                                    <Button
                                        variant="ghost"
                                        className="w-full text-charcoal-light hover:text-charcoal-mid"
                                        onClick={() => setIsOpen(false)}
                                    >
                                        Alışverişe Devam Et
                                    </Button>
