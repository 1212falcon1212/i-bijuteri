...
interface Props {
    ...
    required?: boolean;
    /** Lock the province selector — only district editable */
    lockCity?: boolean;
}
...
export function CityDistrictSelect({ value, onChange, required = false, lockCity = false }: Props) {
    ...
    return (
        <div className="grid grid-cols-2 gap-3">
            <div>
                <label className="block text-[11px] font-medium text-charcoal-mid mb-1">
                    İl{required ? ' *' : ''}
                </label>
                <select
                    value={value.cityId ?? ''}
                    ...
                    className="w-full h-10 px-3 rounded-lg border border-card-border bg-white text-sm focus:outline-none focus:border-slate-900 transition-colors disabled:opacity-60 disabled:bg-surface-bg disabled:cursor-not-allowed"
                >
            ...
            <div>
                <label className="block text-[11px] font-medium text-charcoal-mid mb-1">İlçe</label>
                <select
                    ...
                    className="w-full h-10 px-3 rounded-lg border border-card-border bg-white text-sm focus:outline-none focus:border-slate-900 transition-colors disabled:opacity-50"
                >
