    ...
    return (
        <div className="border border-card-border rounded-xl bg-white overflow-hidden">
            ...
            <button
                ...
                className="w-full flex items-center justify-between p-4 hover:bg-surface-bg transition-colors"
            >
                <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-lg bg-surface-bg flex items-center justify-center overflow-hidden flex-shrink-0">
                        {logo ? (
                            <Image src={logo} alt={name} width={40} height={40} className="object-contain" />
                        ) : (
                            <span className="text-lg font-bold text-charcoal-light">{name.charAt(0)}</span>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-3">
                    {isConnected && (
                        <Badge
                            ...
                            className={cn(
                                "gap-1",
                                isActive && "bg-[#FBF8F3] text-[#B89968] border-[#D4B896]",
                    ...
                    <ChevronDown className={cn("w-5 h-5 text-charcoal-light transition-transform duration-200", expanded && "rotate-180")} />
                </div>
            </button>

            {/* Expanded Content */}
            {expanded && (
                <div className="border-t border-card-border p-4 bg-surface-bg">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                                <div className="bg-white rounded-lg p-3 border border-card-border">
                                    <span className="text-charcoal-light block mb-1">Durum</span>
                                    <span className={cn(
                                        "font-medium",
                                        isActive && "text-[#B89968]",
                                        hasError && "text-red-600",
                                        isPending && "text-amber-600",
                                        !isActive && !hasError && !isPending && "text-charcoal-mid"
                                    )}>
                                        {isActive ? 'Aktif' : hasError ? 'Hata' : isPending ? 'Beklemede' : 'Pasif'}
                                    </span>
                                </div>
                                <div className="bg-white rounded-lg p-3 border border-card-border">
                                    <span className="text-charcoal-light block mb-1">Son Senkronizasyon</span>
                            ...
                            {credentials && (
                                <div className="bg-white rounded-lg p-4 border border-card-border">
                                    <h4 className="text-sm font-medium text-charcoal-mid mb-3 flex items-center gap-2">
                                        ...
                                        {config.fields.map((field) => {
                                            ...
                                            return (
                                                <div key={field.key} className={field.type === 'switch' ? 'sm:col-span-2' : ''}>
                                                    <span className="text-charcoal-light block text-xs mb-0.5">{field.label}</span>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {config.fields.map((field) => (
                                    <div key={field.key} className={field.type === 'switch' ? 'sm:col-span-2' : ''}>
                                        {field.type === 'switch' ? (
                                            <div className="flex items-center justify-between bg-white rounded-lg p-3 border border-card-border">
                                                <Label htmlFor={`${id}-${field.key}`} className="text-sm font-medium text-charcoal-mid">
                                                    {field.label}
                                                </Label>
                                                <Switch
                                                    id={`${id}-${field.key}`}
