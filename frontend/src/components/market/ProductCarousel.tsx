        ...
        <section className="py-4">
            <div className="flex items-center justify-between mb-4 px-1">
                ...
                {linkUrl && (
                    <Link
                        href={linkUrl}
                        className="group flex items-center gap-1.5 text-sm font-semibold border-[1.5px] border-[#D4B896] text-[#B89968] hover:bg-[#B89968] hover:text-white rounded-[10px] px-4 py-1.5 transition-colors"
                    >
                        Tümünü Gör
                    ...
                    {displayProducts.map((product, index) => {
                        ...
                        return (
                            <Link
                                ...
                                className="group relative flex flex-row rounded-3xl border border-[#f0eceb] dark:border-[#2a2a2a] overflow-hidden hover:border-[#D4B896] dark:hover:border-[#B89968] transition-colors"
                            >
                                {/* Left: Product Image */}
                                ...
                                <div className="flex-1 min-w-0 p-4 sm:p-6 flex flex-col justify-center">
                                    {product.brand && (
                                        <p className="text-[12px] font-extrabold text-[#B89968] dark:text-[#D4B896] uppercase tracking-wider mb-2 truncate">
                                            {product.brand}
                                        </p>
                                    )}
                                    <h3 className="text-sm sm:text-base font-semibold text-[#1a1a1a] dark:text-slate-200 line-clamp-2 mb-3 group-hover:text-[#B89968] dark:group-hover:text-[#D4B896] transition-colors">
                                        {product.name}
                                    </h3>
                                    <div className="mb-2">
                                    ...
                                    {offersCount > 0 && (
                                        <p className="text-sm text-[#6b7280]">
                                            <span className="font-semibold text-[#B89968]">{offersCount} ilan</span>
                                        </p>
                                    )}
                                </div>
                            </Link>
    ...
    return (
        <section className="py-4">
            <div className="flex items-center justify-between mb-4 px-1">
                ...
                {linkUrl && (
                    <Link href={linkUrl} className="text-sm text-[#B89968] font-medium">
                        Tümünü Gör
                    </Link>
                )}
            </div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
                    {products.map((product) => (
                        ...
                        >
                            <div
                                className={cn(
                                    "relative rounded-lg overflow-hidden bg-[#F1E6D0] mb-2",
                                    styles.image
                                )}
                            >
                                {(product.image_url || product.image) ? (
                            ...
                            <p
                                className={cn(
                                    "font-medium text-[#1a1a1a] line-clamp-2 group-hover:text-[#B89968] transition-colors",
                                    styles.text
                                )}
                            >
                                {product.name}
                            </p>
                            <p className="text-sm font-bold text-[#B89968] mt-1">
        ...
        <section className="py-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Link
                    ...
                    className="group relative bg-[#F1E6D0] rounded-2xl p-6 overflow-hidden border border-[#f0eceb] hover:border-[#D4B896] hover:shadow-md transition-all"
                >
                    <div className="relative flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <span className="inline-block px-3 py-1 bg-[#F1E6D0] text-[#B89968] text-xs font-bold rounded-sm mb-3">
                                ÖNE ÇIKAN
                            </span>
                            {featured.brand && (
                                <p className="text-sm text-charcoal-light font-medium mb-1">
                                    {featured.brand}
                                </p>
                            )}
                            <h3 className="text-xl font-bold text-[#1a1a1a] mb-2 group-hover:text-[#B89968] transition-colors">
                                {featured.name}
                            </h3>
                            <div className="flex items-baseline gap-2 justify-center md:justify-start mb-4">
                                <span className="text-3xl font-bold text-[#B89968]">
                            ...
                            <span className="inline-flex items-center gap-2 text-sm font-medium text-[#B89968] group-hover:gap-3 transition-all">
                                Detayları Gör
