  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto w-full max-w-md space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {savedCards.map((card) => (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg border p-3 text-left transition-colors ${
                  selectedSavedCard === card.ctoken
                    ? 'border-[#D4B896] bg-[#FBF8F3] dark:border-[#8C6F3F] dark:bg-[#8C6F3F]/30'
              ...
              >
                ...
                >
                  {selectedSavedCard === card.ctoken && (
                    <div className="h-2.5 w-2.5 rounded-full bg-[#FBF8F3]" />
                  )}
                </div>
          ...
          {isUsingSavedCard && (
            <Button
              ...
              size="sm"
              onClick={useNewCard}
              className="text-[#B89968] hover:text-[#B89968] dark:text-[#D4B896]"
            >
              Yeni Kart ile Ode
            </Button>
          )}
      ...
      <div className="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3 dark:bg-gray-800/50">
        <span className="text-sm font-medium text-gray-600 dark:text-gray-400">
          Ödenecek Tutar (Tek Çekim)
        </span>
        <span className="text-lg font-bold text-[#B89968] dark:text-[#D4B896]">
      ...
      <Button
        ...
        className="h-12 w-full bg-[#FBF8F3] text-base font-semibold text-white hover:bg-[#8C6F3F] disabled:opacity-50 dark:bg-[#8C6F3F] dark:hover:bg-[#8C6F3F]"
      >
        {isSubmitting ? (
  ...
  function EmbeddedCardForm({ totalAmount }, ref) {
    ...
    return (
      <div className="space-y-5">
        {/* Card mode tabs */}
        {!loadingSavedCards && savedCards.length > 0 && (
          <div className="flex rounded-lg border border-card-border dark:border-slate-700 overflow-hidden">
            <button
              type="button"
              onClick={() => { if (savedCards[0]) selectSavedCard(savedCards[0].ctoken); }}
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                cardMode === 'saved'
                  ? 'bg-[#FBF8F3] text-[#B89968] dark:bg-[#8C6F3F]/30 dark:text-[#D4B896]'
                  : 'bg-white text-charcoal-mid hover:bg-surface-bg dark:bg-charcoal dark:text-charcoal-light'
              }`}
            >
              Kayıtlı Kart Kullan
            </button>
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors border-l border-card-border dark:border-slate-700 ${
                cardMode === 'new'
                  ? 'bg-[#FBF8F3] text-[#B89968] dark:bg-[#8C6F3F]/30 dark:text-[#D4B896]'
                  : 'bg-white text-charcoal-mid hover:bg-surface-bg dark:bg-charcoal dark:text-charcoal-light'
              }`}
            ...
            {savedCards.map((card) => (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-xl border-2 p-3 text-left transition-all ${
                  selectedSavedCard === card.ctoken
                    ? 'border-[#B89968] bg-[#B89968]/5 dark:border-[#8C6F3F] dark:bg-[#8C6F3F]/30'
                    : 'border-card-border bg-white hover:border-card-border dark:border-slate-700 dark:bg-charcoal'
                }`}
              >
                <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 ${
                  selectedSavedCard === card.ctoken ? 'border-[#B89968]' : 'border-card-border dark:border-slate-600'
                }`}>
                  {selectedSavedCard === card.ctoken && <div className="h-2 w-2 rounded-full bg-[#B89968]" />}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-charcoal dark:text-slate-100">
                    **** **** **** {card.last_4}
                  </p>
        ...
        {/* Total (single payment only) */}
        <div className="flex items-center justify-between rounded-lg bg-surface-bg px-4 py-3 dark:bg-charcoal/50">
          <span className="text-sm font-medium text-charcoal-mid dark:text-charcoal-light">Ödenecek Tutar (Tek Çekim)</span>
          <span className="text-lg font-bold text-[#B89968] dark:text-[#D4B896]">
            {formatPrice(totalAmount)}
          </span>
        </div>
