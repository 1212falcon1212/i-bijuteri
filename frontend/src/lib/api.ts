...
export const authApi = {
  login: (email: string, password: string, revoke_others = false) =>
    api.post<{ user: User; token: string; message: string }>('/auth/login', { email, password, revoke_others }),
  ...
  verifyTaxNumber: (tax_number: string) =>
    api.post<TaxNumberVerificationResult>('/auth/verify-tax-number', { tax_number }),
...
export interface TaxNumberVerificationResult {
  valid: boolean;
...
export interface TopbarSettings {
  ...
  hours_text: string;
  phone: string;
  seller_link_text: string;
  seller_link_url: string;
  contact_link_text: string;
  contact_link_url: string;
...
export interface MarketHomeSeller {
  name: string;
  slug: string;
  city?: string;
  years?: number | string;
...
export interface MarketHomeContent {
  hero: {
    ...
    trustline: string[];
    tag: { label: string; title: string; meta: string };
  };
  trust_strip: { title: string; subtitle: string }[];
  categories_section: {
    eyebrow: string;
    title: string;
    description: string;
...
// Legacy CMS Page Type (kept for cmsApi.getPage compatibility)
export interface LegacyCmsPage {
  id: number;
  title: string;
  slug: string;
  content: string;
...
export const cmsApi = {
  ...
  getPage: (slug: string) => api.get<LegacyCmsPage>(`/cms/pages/${slug}`),
};

// Dynamic Pages (Kurumsal / Legal / Yardım) — backend-driven content
export type CmsPageCategory = 'kurumsal' | 'legal' | 'yardim';
...
export type CmsPageListItem = Pick<CmsPage, 'id' | 'title' | 'slug' | 'excerpt' | 'updated_at'>;

export const pagesApi = {
  getBySlug: (slug: string) =>
    api.get<{ page: CmsPage }>(`/pages/${slug}`),
  byCategory: (category: CmsPageCategory) =>
    api.get<{ pages: CmsPageListItem[] }>(`/pages-by-category/${category}`),
};
