...
export function useMDXComponents(components: MDXComponents): MDXComponents {
    return {
        ...
        a: ({ href, children }) => (
            <Link
                href={href || '#'}
                className="text-[#B89968] hover:text-[#8C6F3F] underline underline-offset-2"
            >
                {children}
            </Link>
        ),
        ...
        blockquote: ({ children }) => (
            <blockquote className="border-l-4 border-[#B89968] pl-4 py-2 my-4 bg-[#FBF8F3] rounded-r-lg">
                {children}
            </blockquote>
        ),
        strong: ({ children }) => (
