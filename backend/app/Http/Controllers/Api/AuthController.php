...
class AuthController extends Controller
{
    ...
    /**
     * Verify a tax number availability for registration form.
     */
    public function verifyTaxNumber(Request $request): JsonResponse
    {
        $request->validate([
            'tax_number' => ['required', 'string', 'regex:/^[0-9]{10,11}$/'],
        ]);

        $alreadyRegistered = User::where('tax_number', $request->input('tax_number'))->exists();

        if ($alreadyRegistered) {
            return response()->json([
                'valid' => false,
                'message' => 'Bu vergi numarasi zaten kayitli.',
    ...
    public function updateProfile(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'default_shipping_cost' => 'nullable|numeric|min:0|max:5000',
        ]);
        ...
        // Only sellers can set default_shipping_cost
        if (isset($validated['default_shipping_cost']) && $user->role !== User::ROLE_SELLER) {
            unset($validated['default_shipping_cost']);
        }
    ...
    public function deactivateAccount(Request $request): JsonResponse
    {
        ...
        // Soft deactivation: Mark account instead of hard delete
        $user->update([
            'is_verified' => false,
            'deactivated_at' => now(),
    ...
    /**
     * Build a consistent user payload for responses.
     *
     * @return array<string, mixed>
     */
    private function userPayload(User $user, bool $includeTimestamps = false): array
    {
        $payload = [
            'id' => $user->id,
            'email' => $user->email,
        ...
        if ($user->role === User::ROLE_SELLER) {
            $payload['default_shipping_cost'] = $user->default_shipping_cost !== null
                ? (float) $user->default_shipping_cost
                : null;
        }
