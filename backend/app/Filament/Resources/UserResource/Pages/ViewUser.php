...
class ViewUser extends ViewRecord
{
    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('approve')
                ...
                ->modalDescription('Bu kullaniciyi onaylamak istediginize emin misiniz?')
                ->action(function () {
                    $this->record->approve(auth()->id());
                    Mail::to($this->record->email)->queue(new SellerApprovedMail($this->record));
                    Notification::make()->title('Kullanici onaylandi')->success()->send();
                }),
            ...
            Actions\Action::make('reject')
                ...
                ->action(function (array $data) {
                    ...
                    Notification::make()->title('Basvuru reddedildi')->warning()->send();
                }),
    ...
    public function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                Components\Section::make('Isletme Bilgileri')
                    ->schema([
                        Components\TextEntry::make('business_name')->label('Isletme Adi'),
                        Components\TextEntry::make('tax_number')->label('Vergi/TC No'),
                        Components\TextEntry::make('email')->label('E-posta'),
                        Components\TextEntry::make('phone')->label('Telefon'),
                        Components\TextEntry::make('city')->label('Sehir'),
                        Components\TextEntry::make('address')->label('Adres'),
                    ])->columns(3),

                Components\Section::make('Dogrulama Durumu')
                    ->schema([
                ...
                Components\Section::make('Yuklenen Belgeler')
                    ->schema([
                        Components\RepeatableEntry::make('sellerDocuments')
                            ...
                            ->schema([
                                ...
                                Components\TextEntry::make('original_name')->label('Dosya Adi'),
